...
//       try {
//            classifierGesture = useModelGesture.getClassifier("//Liuwu//RF0630.model");//在程序在进行初始化的时候，就进行模型的引入
//        } catch (IOException e) {
//            e.printStackTrace();
...
//        }
        ...
        try {
            classifierGesture = useModelGesture.getClassifier(getAssets().open("RF07102.model"));
//			classifier=dataInstance.getClassifier("//Liuwu//baye_choiceMusic.model");
        } catch (IOException e) {
            e.printStackTrace();
        } catch (ClassNotFoundException e) {
            e.printStackTrace();
...
for (String s:f){
    ...
    private void deal() {
        ...
        double[] pred = new  double[7];
        try {
            pred = classifierGesture.distributionForInstance(instancesGesture.instance(instanceNum - 1));
//            System.out.println("-------"+ Utils.arrayToString(classifierGesture.getOptions()));
            for (int o = 0;o<pred.length;o++){
                System.out.println(pred[o]);
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        switch (resultType2[0]) {
            case '0':
                ...
                resultString.append(  "DTW识别结果是：" + String.valueOf(dtwResult) + "        RF识别结果是：O \n");
                showResult.setText(resultString);
                break;
            case '1':
                Toast.makeText(this, String.valueOf(dtwResult), Toast.LENGTH_SHORT).show();
                resultString.append(  "DTW识别结果是：" + String.valueOf(dtwResult) + "        RF识别结果是：C \n");
                showResult.setText(resultString);
                break;
            case '2':
                Toast.makeText(this, String.valueOf(dtwResult), Toast.LENGTH_SHORT).show();
                resultString.append(  "DTW识别结果是：" + String.valueOf(dtwResult) + "        RF识别结果是：e \n");
                showResult.setText(resultString);
                break;
            case '3':
                Toast.makeText(this, String.valueOf(dtwResult), Toast.LENGTH_SHORT).show();
                resultString.append(  "DTW识别结果是：" + String.valueOf(dtwResult) + "        RF识别结果是：V \n");
                showResult.setText(resultString);
                break;
            case '4':
                Toast.makeText(this, String.valueOf(dtwResult), Toast.LENGTH_SHORT).show();
                resultString.append(  "DTW识别结果是：" + String.valueOf(dtwResult) + "        RF识别结果是：U \n");
                showResult.setText(resultString);
                break;
            case '5':
                Toast.makeText(this, String.valueOf(dtwResult), Toast.LENGTH_SHORT).show();
                resultString.append(  "DTW识别结果是：" + String.valueOf(dtwResult) + "        RF识别结果是：W \n");
                showResult.setText(resultString);
                break;
            case '6':
                Toast.makeText(this, String.valueOf(dtwResult), Toast.LENGTH_SHORT).show();
                resultString.append(  "DTW识别结果是：" + String.valueOf(dtwResult) + "        RF识别结果是：y \n");
                showResult.setText(resultString);
                break;
        }
        listClear();
